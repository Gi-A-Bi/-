...
- **전체 톤 통일** — 상세(캐릭터 시트) 히어로와 순위 배너에도 등급 프레임/강조 적용(다이아 홀로그램, 골드 금테 등), 헤더·학급 경험치 배너 색조 정리
- **다이아 홀로그램 애니메이션** — 다이아 등급 카드/히어로의 무지개 테두리가 회전하고 홀로 sheen이 흐름(`prefers-reduced-motion` 시 정지)
- **분할 화면(마스터-디테일)** — 넓은 화면(≥900px)에서 학생 카드를 누르면 새 화면 대신 **오른쪽 패널**에 캐릭터 시트가 열림(선택 카드 강조, 패널 sticky). 좁은 화면은 기존처럼 전체 화면 전환
